...
#repricing the portfolio 
  ...
  #IV matrix shifted by the 1 year ATM IV difference
IV_mat = matrix(NA, nrow = n_sim, ncol = length(K_vec))
for (i in 1:length(K_vec) ) {
  
  IV_mat[ , i] = f_parametric_IV(params = params_vec, m = K_vec[i]/SP500_new,
                                 tau = rep(TT_vec[i], n_sim) )
}
...
for(i in 1:5){
  logrets_vix = as.numeric(fit_vix$coef[2]) + as.numeric(fit_vix$coef[1])*
    logrets_vix + res_vix[( (i-1) * n_sim + 1 ) : (i * n_sim) ]
